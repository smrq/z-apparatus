function ({ caller, calling_script }, args) {
	let authLevel = #fs.symlink.auth();
	if (calling_script !== 'zmachine.play' && authLevel < 1000) {
		return { ok: false };
	}
...
zmachine.play { list: true }
  Lists the stories available to play.

zmachine.play { load: "name" }
  Loads a story, starting from the beginning.

zmachine.play { msg: "open mailbox" }
  Enters the command into the game.


\`AMACRO\`

/z = zmachine.play {{ msg: {0} }}
...
`;
	...
	if (args.list) {
		const stories = #db.f({ type: 'zmachine/story' }, { name: 1, title: 1 })
			.array()
			.sort((a, b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0);
		const maxLength = Math.max.apply(Math, stories.map(story => story.name.length));
		return `
\`ASTORIES\`

${stories.map(story => `\`V${story.name.padEnd(maxLength + 2)}\`${story.title}`).join('\n')}
`;
	}

 	if (args.load) {
		const story = #db.f({ type: 'zmachine/story', name: args.load }).first();
		if (!story) {
			return `\`DStory ${args.load} was not found.\``;
		}

		const opcodeTable = #db.f({ type: 'zmachine/opcodes', version: story.memory[0] }).first().table;
		data = {
 			type: 'zmachine/ram',
 			user: caller,
 			name: args.load,
			state: {
				memory: story.memory,
				stack: [{ stack: [] }],
				pc: read16(story.memory, 0x06),
				opcodeTable
			},
			status: 'new',
			random: args.random
		};
		#db.us({ type: 'zmachine/ram', user: caller }, data);
	} else {
		data = #db.f({ type: 'zmachine/ram', user: caller }).first();
	}

	if (!data) {
		return `\`C[No story is loaded.]\`

${usage}`;
	}

	if (data.status === 'finished') {
		return `\`C[The story has finished.]\`

${usage}`;
	}

	let { cmd, debug } = args;
	let { state, random } = data;
	...
	if (random) {
		...
		random = function random(max) {
			if (data.random.length) {
				const value = randomValues.shift() + 1;
				if (debug === 'inst' || debug === 'random') {
					#D(`RANDOM ${max} -> ${value}`);
				}
				if (value > max) {
		...
		random = function random(max) {
			const value = ((Math.random()*(max-1)) | 0) + 1;
			if (debug === 'inst' || debug === 'random') {
				#D(`RANDOM ${max} -> ${value}`);
			}
			return value;
	...
	let input;
	for (;;) {
		const runState = run(output, input, random);
		input = null;
		if (runState === 'yield') {
			if (cmd.length) {
				input = cmd.shift();
	...
	function persist() {
		#db.u({ type: 'zmachine/ram', user: caller }, { $set: {
			state: state,
			status: data.status,
			random: data.random
		}});
	...
	function run(output, input, random) {
		const instruction = decodeInstruction(state.pc);
		...
		const operands = instruction.operands.map(operand => operand.type === 'constant' ?
				operand.value :
				variableLoad(operand.value));

		if (debug === 'inst') {
			debugInstruction(instruction, operands);
		}
		if (debug === 'zorkmid') {
			debugZorkmid(instruction, operands);
		}

		state.pc = instruction.nextAddress;
		return executeInstruction(instruction, operands, output, input, random);
	}
	...
	function debugZorkmid(instruction, operands) {
	...
	function decodeDictionaryTable() {
		let address = getDictionaryAddress();
		...
		const entries = [];
		const textLength = getVersion() <= 3 ? 6 : 9;
		const encodedTextLength = getVersion() <= 3 ? 4 : 6;
	...
	function decodeText(address, isAbbreviation = false) {
		...
		for (;;) {
			...
			zchars.forEach(c => {
				if (abbr != null) {
					const abbreviationAddress = 2 * read16(state.memory, getAbbreviationsTableAddress() + 2 * (abbr + c));
					text += decodeText(abbreviationAddress, true).text;
					abbr = null;
				} else if (tenbit != null) {
					if (tenbit === -1) {
						tenbit = c;
					...
					if (a === 2 && c === 6) {
						tenbit = -1;
					} else if (version <= 4 || getAlphabetTableAddress() === 0) {
						const char = alphabets[a][c-6];
						text += char;
					} else {
						const index = 26 * a + (c - 6);
						text += String.fromCharCode(state.memory[getAlphabetTableAddress() + index]);
					}
				}
			});
	...
	function getDictionaryAddress() {
		return read16(state.memory, 0x08);
	}

	function getObjectTableAddress() {
		return read16(state.memory, 0x0A);
	}

	function getGlobalTableAddress() {
		return read16(state.memory, 0x0C);
	}

	function getAbbreviationsTableAddress() {
		return read16(state.memory, 0x18);
	}

	function getRoutinesOffset() {
		return read16(state.memory, 0x28);
	}

	function getStaticStringsOffset() {
		return read16(state.memory, 0x2A);
	}

	function getAlphabetTableAddress() {
	...
	function lookupOpcode(code, operandCount) {
	...
	function getObjectBase(objectId) {
		...
		return getObjectTableAddress() + 2 * (maxPropertyId - 1) + objectSize * (objectId - 1);
	}

	function getAttributeBase(objectId, attributeId) {
		const version = getVersion();
		const base = getObjectBase(objectId);
	...
	function getObjectAttribute(objectId, attributeId) {
		const [address, bit] = getAttributeBase(objectId, attributeId);
		return !!(state.memory[address] & (1 << bit));
	}

	function setObjectAttribute(objectId, attributeId, value) {
		const [address, bit] = getAttributeBase(objectId, attributeId);
		if (value) {
	...
	function getObjectRelation(objectId, relationNumber) {
		const version = getVersion();
		const base = getObjectBase(objectId);
		return version <= 3 ?
	...
	function setObjectRelation(objectId, relationNumber, relation) {
		const version = getVersion();
		const base = getObjectBase(objectId);
		if (version <= 3) {
	...
	function moveObject(obj, dest) {
		const parent = getObjectRelation(obj, 0 /*RELATION_PARENT*/);
		if (parent !== 0) {
			const sibling = getObjectRelation(obj, 1 /*RELATION_SIBLING*/);
			let node = getObjectRelation(parent, 2 /*RELATION_CHILD*/);
			if (node === obj) {
				setObjectRelation(parent, 2 /*RELATION_CHILD*/, sibling);
			} else {
				let nodeSibling = getObjectRelation(node, 1 /*RELATION_SIBLING*/);
				while (nodeSibling !== obj) {
					node = nodeSibling;
					nodeSibling = getObjectRelation(node, 1 /*RELATION_SIBLING*/);
				}
				setObjectRelation(node, 1 /*RELATION_SIBLING*/, sibling);
			}
		}

		if (dest !== 0) {
			const destChild = getObjectRelation(dest, 2 /*RELATION_CHILD*/);
			setObjectRelation(obj, 1 /*RELATION_SIBLING*/, destChild);
			setObjectRelation(dest, 2 /*RELATION_CHILD*/, obj);
		} else {
			setObjectRelation(obj, 1 /*RELATION_SIBLING*/, 0);
		}

		setObjectRelation(obj, 0 /*RELATION_PARENT*/, dest);
	}

	function getObjectPropertiesAddress(objectId) {
		const version = getVersion();
		const base = getObjectBase(objectId);
		return read16(state.memory, base + (version <= 3 ? 7 : 12));
	}

	function setObjectPropertiesAddress(objectId, address) {
		const version = getVersion();
		const base = getObjectBase(objectId);
	...
	function getObjectName(objectId) {
		let address = getObjectPropertiesAddress(objectId);
		return decodeText(address + 1).text;
	}

	function getObjectFirstPropertyAddress(objectId) {
		let address = getObjectPropertiesAddress(objectId);
		return address + 1 + 2 * (state.memory[address]);
	}

	function getPropertyNumber(address) {
	...
	function getPropertyDataAddress(address) {
		if ((getVersion() <= 3) || !(state.memory[address] & 0x80)) {
			return address + 1;
		} else {
			return address + 2;
		}
	...
	function getPropertyDataLength(address) {
		if (getVersion() <= 3) {
	...
	function getObjectPropertyDataLengthFromDataAddress(address) {
		address -= 1;
		if (getVersion() <= 3) {
			return 1 + (state.memory[address] >> 5);
		} else if (state.memory[address] & 0x80) {
	...
	function getNextPropertyAddress(address) {
		return getPropertyDataAddress(address) + getPropertyDataLength(address);
	}

	function getObjectPropertyAddress(objectId, propertyId) {
		let address = getObjectFirstPropertyAddress(objectId);
		let id = getPropertyNumber(address);
		while (id > propertyId) {
			address = getNextPropertyAddress(address);
			id = getPropertyNumber(address);
		}
		if (id === propertyId) {
			return address;
		} else {
	...
	function getObjectPropertyDataAddress(objectId, propertyId) {
		const address = getObjectPropertyAddress(objectId, propertyId);
		return address ? getPropertyDataAddress(address) : 0;
	}

	function getObjectPropertyData(objectId, propertyId) {
		const address = getObjectPropertyAddress(objectId, propertyId);
		if (!address) {
			return null;
		}
		const dataAddress = getPropertyDataAddress(address);
		const length = getPropertyDataLength(address);
		return state.memory.slice(dataAddress, dataAddress + length);
	}

	function getObjectFirstPropertyNumber(objectId) {
		const address = getObjectFirstPropertyAddress(objectId);
		return getPropertyNumber(address);
	}

	function getObjectNextPropertyNumber(objectId, propertyId) {
		const address = getObjectPropertyAddress(objectId, propertyId);
		if (address === 0) {
			throw new Error(`tried to get next prop for nonexistent prop ${propertyId} on object ${objectId}`);
		}
		const nextAddress = getNextPropertyAddress(address);
		return getPropertyNumber(nextAddress);
	}

	function setObjectPropertyData(objectId, propertyId, value) {
		const address = getObjectPropertyAddress(objectId, propertyId);
		if (address === 0) {
			throw new Error(`tried to set property data for nonexistent prop ${propertyId} on object ${objectId}`);
		}
		const dataAddress = getPropertyDataAddress(address);
		const dataLength = getPropertyDataLength(address);
	...
	function parseText(text, textBufferAddress, parseTableAddress) {
		let address = textBufferAddress;
		const maxInputLength = state.memory[address++] - (getVersion() <= 4 ? 1 : 0);
		...
		if (getVersion() <= 4 || parseTableAddress !== 0) {
			const dictionary = decodeDictionaryTable();
			const separatorRegex = new RegExp('([ ' + dictionary.wordSeparators.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '])');

			let position = getVersion() <= 4 ? 1 : 2;
			...
			words.forEach(word => {
				const encodedText = encodeText(word.text);
				if (encodedText) {
	...
	function unpackAddress(op, packedAddress) {
		if (getVersion() <= 3) {
		...
		if (getVersion() <= 7) {
			if (op === 'print_paddr') {
				return packedAddress * 4 + getStaticStringsOffset();
			} else {
				return packedAddress * 4 + getRoutinesOffset();
			}
		}
		return packedAddress * 8;
	}

	function performBranch(offset) {
		if (offset === 0) {
			performReturn(0);
		} else if (offset === 1) {
			performReturn(1);
	...
	function performCall(op, packedAddress, args, resultVariable = null) {
		let address = unpackAddress(op, packedAddress);
	...
	function performReturn(value) {
		const frame = state.stack.pop();
		if (frame.resultVariable != null) {
			variableStore(frame.resultVariable, value);
		}
		state.pc = frame.nextAddress;
	}

	function variableLoad(variable) {
		if (variable === 0) {
			...
			return read16(state.memory, getGlobalTableAddress() + 2 * (variable - 0x10));
		}
	}

	function variableStore(variable, value) {
		if (variable === 0) {
			const frame = state.stack[state.stack.length - 1];
			...
			write16(state.memory, getGlobalTableAddress() + 2 * (variable - 0x10), value);
		}
	}
	...
	function decodeInstruction(address) {
		...
		const form =
			(getVersion() >= 5 && opcodeByte === 0xBE) ? 'extended' :
		...
		switch (form) {
			case 'short': {
				...
				result.opcode = lookupOpcode(opcodeByte & 0x0F, is0OP ? '0OP' : '1OP');
				if (!is0OP) {
			...
			case 'long': {
				result.opcode = lookupOpcode(opcodeByte & 0x1F, '2OP');
				operandTypes.push((opcodeByte & 0x40) ? 2 /*TYPE_VARIABLE*/ : 1 /*TYPE_SMALL_CONSTANT*/);
				operandTypes.push((opcodeByte & 0x20) ? 2 /*TYPE_VARIABLE*/ : 1 /*TYPE_SMALL_CONSTANT*/);
			} break;

			case 'variable': {
				result.opcode = lookupOpcode(opcodeByte & 0x1F, (opcodeByte & 0x20) ? 'VAR' : '2OP');
			} break;

			case 'extended': {
				result.opcode = lookupOpcode(state.memory[address++], 'EXT');
			} break;
		}
		...
		result.operands = operandTypes.map(type => {
			if (type === 0 /*TYPE_LARGE_CONSTANT*/) {
				const value = read16(state.memory, address);
				address += 2;
				return { type: 'constant', value };
			} else if (type === 1 /*TYPE_SMALL_CONSTANT*/) {
		...
		if (result.opcode && result.opcode.text) {
			const { text, nextAddress } = decodeText(address);
			result.text = text;
			address = nextAddress;
		}
	...
	function executeInstruction(instruction, operands, output, input, random) {
		...
		switch (op) {
			case 'add': {
				...
				variableStore(instruction.resultVariable, value);
			} break;
			...
			case 'call_1s': {
				/* call_1s
					...
					Stores routine(). */
				performCall(op, operands[0], [], instruction.resultVariable);
			} break;
			...
			case 'call_2n': {
				...
				performCall(op, operands[0], [operands[1]]);
			} break;
			...
			case 'call_2s': {
				/* call_2s
					...
					Stores routine(arg1). */
				performCall(op, operands[0], [operands[1]], instruction.resultVariable);
			} break;
			...
			case 'call_vn': {
				...
				if (packedAddress !== 0) {
					performCall(op, packedAddress, args);
				}
			} break;
			...
			case 'call_vs': {
				...
				const [packedAddress, ...args] = operands;
				if (packedAddress === 0) {
					variableStore(instruction.resultVariable, 0);
				} else {
					performCall(op, packedAddress, args, instruction.resultVariable);
				}
			} break;
			...
			case 'check_arg_count': {
				...
				if (test === instruction.branchIf) {
					performBranch(instruction.branchOffset);
				}
			} break;
			...
			case 'clear_attr': {
				...
				setObjectAttribute(operands[0], operands[1], false);
			} break;
			...
			case 'dec': {
				...
				let value = variableLoad(operands[0]);
				value = (value - 1) & 0xFFFF;
				variableStore(operands[0], value);
			} break;
			...
			case 'get_child': {
				/* get_child
					...
					0). */
				const child = getObjectRelation(operands[0], 2 /*RELATION_CHILD*/);
				variableStore(instruction.resultVariable, child);
			...
			case 'get_next_prop': {
				...
				const value = (operands[1] === 0) ?
					getObjectFirstPropertyNumber(operands[0]) :
					getObjectNextPropertyNumber(operands[0], operands[1]);
				variableStore(instruction.resultVariable, value);
			} break;
			...
			case 'get_parent': {
				...
				const parent = getObjectRelation(operands[0], 0 /*RELATION_PARENT*/);
				variableStore(instruction.resultVariable, parent);
			} break;
			...
			case 'get_prop': {
				...
				const data = getObjectPropertyData(operands[0], operands[1]);
				if (data) {
					variableStore(instruction.resultVariable, data.length === 1 ?
						data[0] :
						read16(data, 0));
				} else {
					variableStore(instruction.resultVariable, getDefaultPropertyData(operands[1]));
				}
			} break;
			...
			case 'get_prop_addr': {
				...
				const address = getObjectPropertyDataAddress(operands[0], operands[1]);
				variableStore(instruction.resultVariable, address);
			} break;
			...
			case 'get_prop_len': {
				...
				const length = getObjectPropertyDataLengthFromDataAddress(operands[0]);
				if (!length) {
					throw new Error(`object ${objectId} does not have property ${propertyId}`);
				}
				variableStore(instruction.resultVariable, length);
			} break;
			...
			case 'get_sibling': {
				...
				const sibling = getObjectRelation(operands[0], 1 /*RELATION_SIBLING*/);
				variableStore(instruction.resultVariable, sibling);
			...
			case 'insert_obj': {
				/* insert_obj
					...
					zero.) */
				moveObject(operands[0], operands[1]);
			} break;
			...
			case 'jin': {
				...
				const test = getObjectRelation(operands[0], 0 /*RELATION_PARENT*/) === operands[1];
				if (test === instruction.branchIf) {
					performBranch(instruction.branchOffset);
				}
			} break;
			...
			case 'load': {
				...
				const value = variableLoad(operands[0]);
				variableStore(instruction.resultVariable, value);
			} break;
			...
			case 'print_addr': {
				...
				print(decodeText(operands[0]).text);
			} break;
			...
			case 'print_obj': {
				...
				print(getObjectName(operands[0]));
			} break;
			...
			case 'print_paddr': {
				...
				const address = unpackAddress(op, operands[0]);
				const { text } = decodeText(address);
				print(text);
			} break;
			...
			case 'put_prop': {
				...
				setObjectPropertyData(operands[0], operands[1], operands[2]);
			} break;
			...
			case 'read': {
				...
				parseText(input, operands[0], operands[1]);
				if (getVersion() >= 5) {
					variableStore(instruction.resultVariable, 10); // assume RETURN terminated this line
				}
			} break;
			...
			case 'save_undo': {
				/* save_undo
					...
					quick.) */
				// TODO
				variableStore(instruction.resultVariable, (-1)&0xFFFF);
			} break;
			...
			case 'set_attr': {
				...
				setObjectAttribute(operands[0], operands[1], true);
			} break;
			...
			case 'store': {
				...
				variableStore(operands[0], operands[1]);
			} break;
			...
			case 'test_attr': {
				...
				const test = getObjectAttribute(operands[0], operands[1]);
				if (test === instruction.branchIf) {
					performBranch(instruction.branchOffset);
				}
			} break;
