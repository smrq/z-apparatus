function ({ caller }, args) {
	...
	if (data.yielded) {
		data.yielded = false;
		run(data.state, outputBuffer, data.cmd);
	}

	for (;;) {
		const yielded = run(data.state, outputBuffer);
		if (yielded) {
			// TODO add status line
			#db.u({ type: 'ram', user: caller }, { state: data.state, yielded: true });
			return outputBuffer.text;
		}
	}

	function run(state, outputBuffer, input) {
		...
		return executeInstruction(state, instruction.opcode.op, operands, input);
	}

	function executeInstruction(state, op, operands, input) {
		switch (op) {
			case 'add': {
				/* add
			...
			case 'read': {
				...
				const maxInputLength = state.memory[result.operands[0]] - (state.header.version <= 4 ? 1 : 0);
				input = input.toLowerCase().replace(/\n.*/g, '').slice(0, maxInputLength);
				const view = new DataView(state.buffer,
					result.operands[0] + (state.header.version <= 4 ? 1 : 2),
					maxInputLength + (state.header.version <= 4 ? 1 : 0));
				for (let i = 0; i < input.length; ++i) {
					view.setUint8(i, input.charCodeAt(i));
				}
				if (state.header.version <= 4) {
					view.setUint8(input.length, 0);
				}
			} break;
