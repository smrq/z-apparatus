...
static void execute(void) {
  ...
  for(;;) {
    ...
    if(tracing_execution) {
      int stackvalue=(dstackptr==(cstackptr?cstack[cstackptr-1].dstackptr:0))?0:dstack[dstackptr-1];
      fprintf(stderr,"<%06x %02x %d %04x %04x %04x %04x> | %04x | ",instpc,x,argn,args[0]&0xFFFF,args[1]&0xFFFF,args[2]&0xFFFF,args[3]&0xFFFF,stackvalue&0xFFFF);
      for(int n=1;n<=cstack[cstackptr-1].numlocals;n++) {
        fprintf(stderr,"%04x ",cstack[cstackptr-1].locals[n]&0xFFFF);
      }
      fprintf(stderr,"\n");
    }
